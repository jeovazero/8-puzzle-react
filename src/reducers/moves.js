...
const resolveSetMove = (state) => {
  ...
  const resolvePositionDelta = ({digit, delta}) =>
    digit == 0 ? { pos: targetPosition, delta: sumCoordinates(delta, shift) } :
    digit == targetDigit ? { pos: blankPosition, delta: sumCoordinates(delta, scalarMult(-1, shift)) } :
    {};
